...
#### 13. Причина неработоспособности механизма заключается в том, что SELinux блокировал доступ к обновлению файлов на ВМ ns01 для DNS, а также к некоторым файлам, к которым DNS обращается в процессе работы. Данную проблему можно решить двумя способами:
- поменять контекст безопасности (продемонстрировано)
- скомпилировать новые модули для selinux и применить их в ядре (опробовано).

#### 13. Установил пакет settroubleshoot-server, проанализировал audit.log на ВМ ns01. Программа предлагает следующее решение проблемы: скомпилировать новые модули для SELinux my-iscworker.pp

#### 14. Модули скомпилированы и добавлены в ядро. Ошибки в ВМ по сервису DNS отсутствуют.